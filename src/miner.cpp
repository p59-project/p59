...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("P59Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("P59Miner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("P59Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("p59-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running P59Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
