...
#if defined(NDEBUG)
#error "P59 cannot be compiled without assertions."
#endif
...
//Check Dev payment transaction
bool CheckDevPayTransaction(const CTransaction& tx, int nHeight) {
	CAmount nReward = GetBlockValue(nHeight - 1 );
	CAmount requiredDevPayment=GetDevPayment(nHeight - 1, nReward);
    CScript rdevScriptPubKey = GetScriptPubKeyFromAddr(devAddr);
    bool devpayfound = false;
    if (devPaymentForce <= nHeight) {
		 BOOST_FOREACH (const CTxOut& txout, tx.vout) { 

			if ( rdevScriptPubKey == txout.scriptPubKey) {
				if(txout.nValue + 0.00001*COIN >= requiredDevPayment)
					devpayfound = true;
			} 
		}
		if (!devpayfound) {
			//LogPrintf("Transaction is without DEV payment\n");
			return false;
		}
		else return true;
	}
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 348100 * COIN;
    } else if (nHeight < Params().LAST_POW_BLOCK()  && nHeight > 0) {
		nSubsidy = 1 * COIN;
    } else if (nHeight < 21600 && nHeight >= Params().LAST_POW_BLOCK()) {
        nSubsidy = 1 * COIN;
    } else if (nHeight < 26640 && nHeight >= 21600) {
        nSubsidy = 1.1 * COIN;
    } else if (nHeight < 31680 && nHeight >= 26640) {
        nSubsidy = 1.2 * COIN;
    } else if (nHeight < 36720 && nHeight >= 31680) {
        nSubsidy = 1.3 * COIN;
    } else if (nHeight < 41760 && nHeight >= 36720) {
        nSubsidy = 1.4 * COIN;
    } else if (nHeight < 46800 && nHeight >= 41760) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight < 51840 && nHeight >= 46800) {
        nSubsidy = 1.6 * COIN;
    } else if (nHeight < 56880 && nHeight >= 51840) {
        nSubsidy = 1.7 * COIN;
    } else if (nHeight < 61920 && nHeight >= 56880) {
        nSubsidy = 1.8 * COIN;
    } else if (nHeight < 66960 && nHeight >= 61920) {
        nSubsidy = 1.9 * COIN;
    } else if (nHeight < 72000 && nHeight >= 66960) {
        nSubsidy = 2.0 * COIN;
    } else if (nHeight < 77040 && nHeight >= 72000) {
        nSubsidy = 1.8 * COIN;
    } else if (nHeight < 82080 && nHeight >= 77040) {
        nSubsidy = 1.6 * COIN;
    } else if (nHeight < 99360 && nHeight >= 82080) {
        nSubsidy = 1.4 * COIN;
    } else if (nHeight < 120960 && nHeight >= 99360) {
        nSubsidy = 1.2 * COIN;
    } else if (nHeight < 146880 && nHeight >= 120960) {
        nSubsidy = 1.0 * COIN;
    } else if (nHeight < 177120 && nHeight >= 146880) {
        nSubsidy = 0.8 * COIN;
    } else if (nHeight < 525600 && nHeight >= 177120) {
        nSubsidy = 0.59 * COIN;
    } else if (nHeight < 33638400 && nHeight >= 525600) {
        nSubsidy = 0.59 * COIN;
		for (int i=0; i<nHeight/525600; i++) {
			nSubsidy=nSubsidy+(nSubsidy*35)/1000;
		}	
    } else if (nHeight >= 33638400) {
        nSubsidy = 4.9730 * COIN;
		for (int i=0; i<(nHeight-33638400)/525600; i++) {
			nSubsidy=nSubsidy-(nSubsidy*35)/1000;
		}	
    } else {
        nSubsidy = 0 * COIN;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Check dev rewards
	bool cDevReward=false;
    BOOST_FOREACH (const CTransaction& tx, block.vtx) 
        if (CheckDevPayTransaction(tx, nHeight)) cDevReward=true;
	if (!cDevReward) {
		LogPrintf("CheckBlock() : DEV payment missing, TRANSACTION CHECK FAILED\n");
		return state.DoS(0, error("CheckBlock() : DEV payment missing"), REJECT_INVALID, "bad-cb-payee");
	}
