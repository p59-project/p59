...
*Setup instructions for a gitian build of P59 using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the P59
...
Clone the git repositories for p59 and gitian and then checkout the p59 version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/p59-crypto/p59.git
cd p59
...
Follow the instructions in [doc/release-process.md](release-process.md) in the p59 repository
...
To build P59 (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the p59 repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/p59/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/p59-crypto/p59
...
For example:
```bash
URL=https://github.com/crowning-/p59.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit p59=${COMMIT} --url p59=${URL} ../p59/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit p59=${COMMIT} --url p59=${URL} ../p59/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit p59=${COMMIT} --url p59=${URL} ../p59/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/p59-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/p59-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/p59-build.assert
```
...
[p59/gitian.sigs](https://github.com/p59-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
